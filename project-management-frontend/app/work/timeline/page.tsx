...
export default function WorkTimelinePage() {
  ...
  const myTasks = tasks
    .filter((t) => t.assigned_to === userId && t.start_date)
    .sort((a, b) => new Date(a.start_date!).getTime() - new Date(b.start_date!).getTime())

  const months = Array.from(new Set(myTasks.map((t) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground">Cronologia de tus tareas asignadas</p>
      </div>

      {months.length === 0 ? (
        <Card><CardContent className="py-12 text-center text-muted-foreground">No tienes tareas con fecha de inicio.</CardContent></Card>
      ) : (
                    ...
                    <Card key={t.id}>
                      <CardContent className="flex items-center gap-4 p-3">
                        ...
                        <div className="text-right text-xs text-muted-foreground shrink-0">
                          ...
                          {t.due_date && <div>- {new Date(t.due_date).toLocaleDateString("es-ES", { day: "numeric", month: "short" })}</div>}
                        </div>
                      </CardContent>
                    </Card>
                  ))}
